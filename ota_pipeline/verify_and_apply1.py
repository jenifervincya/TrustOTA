...
from compress_utils import decompress_patch  # noqa: E402
...
def apply_signed_update(package_path: str, running_firmware_path: str, output_path: str) -> bool:
    ...
    shipped_patch_path = os.path.join(extract_dir, "patch.bin")
    ...
    try:
        ...
        # --- Decompress + apply patch ---
        print("\n[APPLY] Decompressing patch...")
        if manifest.get("compression") == "zlib":
            patch_path = os.path.join(extract_dir, "patch_raw.bin")
            decompress_patch(shipped_patch_path, patch_path)
        else:
            patch_path = shipped_patch_path

        print("[APPLY] Reconstructing new firmware from patch...")
